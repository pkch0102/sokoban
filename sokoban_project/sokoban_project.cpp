...
int i = -1, j, k, stage = 0, clear_num, undo_reset_num = 0, first_com_error = 0;
...
void getmap() {    //map.txt에서 맵 로드
	...
	for (i = 0; i < 5; i++) {
		for (j = 0; j < 30; j++) {
			for (k = 0; k < 30; k++) {
				map_current[i][j][k] = map_data[i][j][k];
			}
		}
	}
...
void pl_move() {
	...
	case 'l':
		dl_x = 1, dl_y = 0;
		if (map_data[stage][pl_y][pl_x] == 4) {    //있던 자리가 박스 놓는 위치일 경우
...
void other_act() {
	...
	case 'r':    //현재 맵 처음부터 다시 시작
		for (j = 0; j < 30; j++) {
			for (k = 0; k < 30; k++) {
				map_current[stage][j][k] = map_data[stage][j][k];
			}
		}
		game_act = 'r';
		undo_reset_num = 0;
...
void clear() {    //게임을 클리어 했는지 판단하는 함수
	...
	if (stage != 5 && clear_num == 0 && map_data[stage][pl_y + dl_y][pl_x + dl_x] != 4) {    //박스 놓는 곳이 없고 플레이어 위치가 박스 놓는 곳이 아니라면 클리어
		stage++;
		undo_reset_num = 0;
		getch();
	}
